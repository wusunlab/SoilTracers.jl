...
let water_density t =
  ...
  -. (6.74694450e5 *. (theta ** (110.0 /. 3.0))) )
  *. rho_crit

let water_dissoc t =
  let n = 6.0 in
  let rho_w = water_density t *. 1e-3 in
  (* convert to g cm^-3 here *)
  let t_sq = t *. t in
  let z =
    rho_w
    *. exp
         ( -0.864671 +. (8659.19 /. t)
         -. (22786.2 /. t_sq *. (rho_w ** (2.0 /. 3.0))) )
  in
  let pK_w_G =
    0.61415 +. (48251.33 /. t) -. (67707.93 /. t_sq)
    +. (10102100.0 /. (t *. t_sq))
  in
  -2.0 *. n
  *. ( log10 (1.0 +. z)
